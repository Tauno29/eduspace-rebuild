...
export default function HomePage() {
  ...
  return (
    <AppFrame className="home-screen">
      ...
      <section className="region-section"><div className="section-heading-row"><h2>Explore by region</h2>{data?.regions?.length ? <button onClick={() => navigate("/regions")}>View all <span>→</span></button> : null}</div>{data?.regions?.length ? <div className="region-grid">{data.regions.map((region) => <RegionCard key={region.id} {...region} onClick={() => navigate(`/region/${region.id}`)} />)}</div> : <DataNotice onRetry={reload} title={status === "loading" ? "Loading live regions" : "Live regions unavailable"} copy={status === "loading" ? "Fetching the latest region records." : "Connect an authoritative EduSpace data source to display current regional school records."} />}</section>
    </AppFrame>
  );
}
